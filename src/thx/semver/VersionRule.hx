...
abstract VersionRule(VersionComparator) from VersionComparator to VersionComparator
{
		...
		var ors = s.split("||").map(function(comp)
		{
			...
			return if (p.length == 1)
			{
				...
				if (p.length == 1)
				{
					...
					else
					{
						...
						switch [VERSION.matched(1), v.length]
						{
							...
							case ["^", 3]:
								var version = Version.arrayToVersion(vf).withPre(VERSION.matched(5), VERSION.matched(6));
								AndRule(GreaterThanOrEqualVersion(version),
				...
				else if (p.length == 2)
				{
					...
					var lp = VERSION.matched(1), lva = versionArray(VERSION), lvf = lva.concat([0, 0, 0])
						.slice(0, 3), lv = Version.arrayToVersion(lvf).withPre(VERSION.matched(5), VERSION.matched(6));

					if (lp != ">" && lp != ">=")
						throw 'invalid left parameter version prefix "${p[0]}", should be either > or >=';
					if (!VERSION.match(p[1]))
						throw 'left hand parameter is not a valid version rule "${p[0]}"';
					var rp = VERSION.matched(1), rva = versionArray(VERSION), rvf = rva.concat([0, 0, 0])
						.slice(0, 3), rv = Version.arrayToVersion(rvf).withPre(VERSION.matched(5), VERSION.matched(6));
					if (rp != "<" && rp != "<=")
			...
			else if (p.length == 2)
			{
				...
				var rva = versionArray(VERSION), rv = Version.arrayToVersion(rva.concat([0, 0, 0]).slice(0, 3))
					.withPre(VERSION.matched(5), VERSION.matched(6));
	...
	static function versionArray(re:EReg):Array<Int>
	{
		var arr:Array<Int> = [];
		var t:String;
		for (i in 2...5)
